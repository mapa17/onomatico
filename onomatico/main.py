...
from onomatico.utils.Names import Names, NamesDataset
...
def train(
    ...
    for i, (data, targets) in enumerate(train_data):
        ...
        output = model(
            data, src_mask
        )  # output = [sequence_length, batch_size, emb_dim]
...
def load_model(
    path: Path, model_cls: Callable, optimizer_cls: Callable, device: str
) -> Dict[str, None]:
    ...
    if src_dev == "cuda" and dest_dev == "cpu":
        ...
        model.load_state_dict(
            checkpoint["model_state_dict"], map_location=torch.device("cuda:0")
        )
...
def _generate_names(
    ...
    for batch_idx in range(num_batches + 1):
        ...
        for input_seq_length in range(1, max_iterations + 1):
            ...
            logits = model(
                seqs, mask
            )  # Logits have the shape [seq_length, batch_size, emb_dim]

            # Use the model output (token probability) of the last entry in the sequence to sample concrete tokens.
            token_dist = torch.nn.functional.softmax(
                logits[-1, :], dim=1
            )  # token_dist = [batch_size, emb_dim]
            next_token = torch.multinomial(
                token_dist, 1
            ).T  # next_token = [1, batch_size]
        ...
        batch_names = [
            "".join(vocab.lookup_tokens(tokens[1:sl]))
            for tokens, sl in zip(token_idx, seq_length)
        ]
...
def tune(
    ...
    # Generate names
    syn_names = _generate_names(
        best_model,
        vocab,
        batch_size=128,
        num_names=tgt_names.shape[0],
        max_iterations=100,
        device=device,
    )
    ...
    names_table = wandb.Table(
        columns=["name"], data=[[n,] for n in syn_names["name"].tolist()]
    )
...
@app.command()
def vocab(data: Path, storage: Path) -> None:
...
def train(
    ...
    # Make sure to use the same vocab for both training and validation
    vocab = torch.load(vocab_storage)
    trn_data = Names(data / "training.csv", mini_batch, device, vocab=vocab)
    val_data = Names(data / "validation.csv", mini_batch, device, vocab=vocab)
